...
var validType = regexp.MustCompile(`^.*\.(yaml|tgz)$`)
...
func NewLocalFilesystemBackend(rootDirectory string) Backend {
...
func LocalBackendFromConfig() Backend {
	return Backend(NewLocalFilesystemBackend(vars.StorageBackendRootDir))
}
...
func (b LocalFilesystemBackend) GetObject(path string) (Object, error) {
	...
	if checkTypeTgz(path) {
		object.ContentType = types.PackageContentType
	} else {
		object.ContentType = types.YamlContentType
	}
	return object, err
}
...
func checkTypeTgz(filename string) bool {
	if validType.MatchString(".tgz") {
		return true
	}
	return false
}
